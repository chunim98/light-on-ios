...
final class AddressButtonForm: TPFormBase {
    ...
    let cityButton = {
        let button = Button()
        button.iconLabelStack.titleLabel.config.text = "도/시/군"
        return button
    }()
    
    let townButton = {
        let button = Button()
        button.iconLabelStack.titleLabel.config.text = "읍/면/동"
        return button
    }()
